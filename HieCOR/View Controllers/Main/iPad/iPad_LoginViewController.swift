//
//  iPad_LoginViewController.swift
//  HieCOR
//
//  Created by HyperMacMini on 22/01/18.
...
class iPad_LoginViewController: BaseViewController, SwipeAndSearchDelegate {
    
    //MARK: IBOutlets    // newCode
    ...
    override func viewDidLoad() {
        ...
        if DataManager.userTypeFlag == true{
            ...
            titleLable.text = "Welcome to HieCOR POS"
            
        }else{
            homePageLogo.isHidden = true
            baseUrlTextField.placeholder = "Site URL"
            titleLable.text = "Log in to your account of POS "
            headingTitleLbl.text = "Let's Get Started"
...
extension iPad_LoginViewController: UITextFieldDelegate {
...
//MARK: API Methods
extension iPad_LoginViewController {
    
    func appendBaseUrl() {
        
        var strBase = "https://"
        if self.baseUrlTextField.text!.contains("outletmagic.com") {
            strBase = "http://"
        }
        if self.baseUrlTextField.text!.contains(strBase)
        {
            BaseURL = self.baseUrlTextField.text!.trimmingCharacters(in: .whitespaces)
        }
        else
        {
            BaseURL = strBase + self.baseUrlTextField.text!.trimmingCharacters(in: .whitespaces)
        }
    ...
    func postAction() {
        let Url = String(format: "\(BaseURL)/rest/v1/auth/login")
        guard let serviceUrl = URL(string: Url) else { return }
        let parameterDictionary = ["username" : self.usernameTextField.text!, "password" : self.passwordTextField.text!]
        var request = URLRequest(url: serviceUrl)
        request.httpMethod = "POST"
        //["Content-Type": "application/json", "X-USERNAME": DataManager.userName, "X-AUTH-KEY": DataManager.authKey, "X-AGENT-ID": DataManager.userID, "device_id": appDelegate.strDeviceUDID]
    ...
    func CheckCRMSettings() {
        ...
        if BaseURL != DataManager.loginBaseUrl && !checkPreviousloginBaseUrl(){
            ...
            HomeVM.shared.customerStatusArray.removeAll()
        }
    }
}
            ...
            if success == 1 {
                ...
                for data in HomeVM.shared.settingList {
                    ...
                    case "pos_pax_gift_card_functionality":
                        if valueData == "true" {
                            //self.arraySelectedPaymet.append("     GIFT")
...
//                            DataManager.isInternalGift = false
                    ...
                    case "pos_pax_debit_card_functionality":
                        if valueData == "true" {
                            //self.arraySelectedPaymet.append("     DEBIT")
...
//                            self.arraySelectedPaymet.append("CREDIT")
                    ...
                    case "pos_line_item_discount":
                        ...
                        var arr = valueData!.components(separatedBy: ",")
