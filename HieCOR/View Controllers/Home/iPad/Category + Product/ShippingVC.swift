...
//  HieCOR
//
//  Created by Sudama dewda on 23/07/19.
...
class ShippingVC: BaseViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        self.tfSelectService.isHidden = false
        ...
        if DataManager.selectedCarrierName != "" && DataManager.selectedCarrierName != nil {
            ...
            if !rateCarrier.contains(DataManager.selectedCarrierName!) {
                ...
                self.tfSelectService.isHidden = true
    ...
    func callAPItoGetShippingCarrierList() {
        HomeVM.shared.getShippingCarrierList(responseCallBack: {(success, message, error) in
            if success == 1 {
                ...
                if self.tfSelectService.isHidden {
                    return
                }
...
extension ShippingVC: HieCORPickerDelegate {
        ...
        if ischeckCarrier {
            ...
            if array_ShippingCarrierList.count > 0 {
                ...
                if !checkRateCarrier(index: pickerView.selectedRow(inComponent: 0)) {
                    ...
                    tfSelectService.isHidden = true
                    labErrorMessae.isHidden = true
                }else {
                    stackView1.isHidden = false
                    stackView2.isHidden = false
                    tfSelectService.isHidden = false
    ...
    func didSelectPickerViewAtIndex(index: Int) {
       ...
       /* if ischeckCarrier {
            ...
            let ob = array_ShippingCarrierList[index].carrier_display_name
            DataManager.selectedCarrier = ob
            DataManager.selectedFullfillmentId = array_ShippingCarrierList[index].fullfillment_id
            pickerTextfield.text = "\(String(describing: DataManager.selectedCarrier!))"
...
//                    self.showAlert(message: "Please fill dimensions")
            ...
            let obDes = array_ShippingServiceList[index].desc
            obRate = array_ShippingServiceList[index].rate
            DataManager.selectedShippingRate = obRate
            let ShippingServiceName = obDes + "($\(obRate))"
            DataManager.selectedService = ShippingServiceName
            pickerTextfield.text = "\(String(describing: DataManager.selectedService!))"
...
extension ShippingVC: EditProductDelegate {
...
//                    self.showAlert(message: "Please fill dimensions")
                    appDelegate.showToast(message: "Please fill dimensions")
                }else if DataManager.selectedCarrier != "" ||  DataManager.selectedCarrier != nil  {
                   // callApi()
                }
                //callApi()
            }
