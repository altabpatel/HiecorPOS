//
//  iPadReturnViewController.swift
//  HieCOR
//
//  Created by Deftsoft on 24/08/18.
...
class iPadRefundViewController: BaseViewController {
    ...
    override func didRotate(from fromInterfaceOrientation: UIInterfaceOrientation) {
        selectTransactionTextField.updateCustomBorder()
        txt_Notes.updateCustomBorder()
        tf_partialAmount.updateCustomBorder()
    }
    ...
    @IBAction func btn_2PartialRefund(_ sender: Any)
    {
        ...
        if isAction == true
        {
            ...
            if(isbtn_2PartialRefundAction == false)
            {
                ...
                btn_4Check.setImage( UIImage(named:"checkbox-unchecked"), for: .normal)
                tf_partialAmount.isHidden = false
                tf_partialAmount.becomeFirstResponder()
            }
            else
            {
                isbtn_1FullRefundAction = false
    ...
    @IBAction func btn_RefundAction(_ sender: Any)
    {
        ...
        if transactionList.count > 0 {
            if selectTransactionTextField.text == "" {
                selectTransactionTextField.setCustomError(text: "Please select Transaction.",bottomSpace: 5)
                return
            }
        }
        ...
        if isbtn_2PartialRefundAction && tf_partialAmount.text == "" {
            tf_partialAmount.setCustomError(text: "Please enter Amount.", isRightSide: true)
            return
        }
        ...
        if txt_Notes.text == "" {
            txt_Notes.setCustomError(text: "Please enter Note.",bottomSpace: 0)
            return
        }
...
extension iPadRefundViewController: UITextFieldDelegate {
...
extension iPadRefundViewController: UITextViewDelegate {
...
extension iPadRefundViewController: HieCORPickerDelegate {
...
//MARK: API Methods
extension iPadRefundViewController {
        ...
        OrderVM.shared.refundOrder(parameters: parameters, responseCallBack: { (success, message, error) in
            if success == 1 {
                self.showAlert(title: "Alert", message: "Refund initiated successfully.", otherButtons: nil, cancelTitle: "Okay", cancelAction: { (action) in
                    self.orderInfoDelegate?.didProductRefunded?()
                    self.dismiss(animated: true, completion: nil)
                })
            }else {
                if message != nil {
...
//                self.transactionModel = OrderVM.shared.transactionInfo
                ...
                if self.transactionListArray.count > 0 {
                    if self.selectedTransactionId == "" {
                        ...
                        let newTotal = Double(self.total) ?? 0.0
                        self.totalLabel.text = "$" + newTotal.roundToTwoDecimal
                    }else {
