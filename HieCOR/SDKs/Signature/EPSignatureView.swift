...
protocol EPSignatureViewDelegate {
    func didStartTouch()
}
...
open class EPSignatureView: UIView {
    ...
    // MARK: - Public Vars
    var delegate: EPSignatureViewDelegate?
    ...
    override open func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        bezierPath.lineWidth = strokeWidth
        NotificationCenter.default.post(name: NSNotification.Name(rawValue: "notificationNameSignatureStart"), object: nil, userInfo: nil)

        delegate?.didStartTouch()
    ...
    override open func touchesMoved(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if let currentPoint = touchPoint(touches) {
            ...
            if bezierCounter == 4 {
                ...
                appDelegate.localBezierPath = bezierPath
                setNeedsDisplay()
    ...
    override open func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
        bezierCounter = 0
        NotificationCenter.default.post(name: NSNotification.Name(rawValue: "notificationNameSignatureCheck"), object: nil, userInfo: nil)
    }
    ...
    */
     ...
     */
    open func getSignatureAsImage(Size:CGFloat) -> UIImage? {
        if isSigned {
            UIGraphicsBeginImageContext(CGSize(width: self.bounds.size.width, height:Size))
    ...
    open func loadSignatureLocal(_ bPath: UIBezierPath) {
        isSigned = true
        bezierPath = bPath
        setNeedsDisplay()
    }
